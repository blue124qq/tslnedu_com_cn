...
(function($) {
    ...
    // 如果只使用默认皮肤，可以不填写skin
    //['default', 'chrome', 'facebook', 'aero']
    d.skin = 'default';
    d.loadingTip = '正在加载...';
    d.showTemp = 1000000;
    d.path = d.path.lastIndexOf('/') < 0 ? '.' : d.path.substring(0, d.path.lastIndexOf('/')) + '/css/artDialog';    
    //扩展dialog
    $.fn.dialog = function(options, yesFn, noFn) {
        if($(this).length<=0)
        return this;
        options = options || {};
        options.id = options.id || this.id || "spb_dialog";
        options.cache = options.cache || false;
        //对a标签支持点击直接打开对话框
        if ($(this).is("a") && $(this).attr("href")) {
            $(this).click(function() {
                art.dialog.load($(this).attr("href"), options, options.cache);
                return false;
            });
            return this;
        }
    ...
    $.dialog.closeAll = function() {
        $.each(art.dialog.dialogList, function() {
            this.close();
        });
    }    
  //表单验证出错插件
  $.dialog.showErrors = function (errors) {
    var statusMessage = "";
    var i = 1;
    ...
    if (statusMessage != "") {
        $.dialog({icon: 'error', id: "statusMessage", title: "\u672A\u901A\u8FC7\u9A8C\u8BC1", 
            content: statusMessage, yesFn: true
        });
      }
    }

    $("form.artDialog").live("submit",function(){
    var self=this;
        $(self).ajaxSubmit({
                    cache: false,
                    ...
                    success: function(data) {                        
                        var dialog=$.dialog.get($(self).parents(".aui_dialog_wrap").attr("id"));
                        if(dialog)
